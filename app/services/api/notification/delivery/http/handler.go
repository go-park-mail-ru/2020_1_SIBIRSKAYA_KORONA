...
func CreateHandler(router *echo.Echo, useCase notification.UseCase, mw *middleware.Middleware) {
	...
	router.GET("/api/notifications", handler.GetAll, mw.CheckAuth, mw.CSRFmiddle)
	router.PUT("/api/notifications", handler.UpdateAll, mw.CheckAuth, mw.CSRFmiddle)
	router.DELETE("/api/notifications", handler.DeleteAll, mw.CheckAuth, mw.CSRFmiddle)
}
